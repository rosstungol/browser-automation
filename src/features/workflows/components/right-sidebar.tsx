...
function NodeIcon({ type, className }: { type: NodeType; className?: string }) {
	...
	return (
		...
		>
			<HugeiconsIcon icon={def.icon} size={14} />
		</span>
	)
}
...
function Section({
	...
	return (
		<div className='flex min-h-0 flex-1 flex-col'>
			<div className='flex items-center gap-2 border-border border-y bg-card px-3 py-1.5 font-semibold text-sm'>
				{icon}
				{title}
			</div>
	...
	if (!node) {
		return (
			<Section title='Editor'>
				<p className='p-3 text-muted-foreground text-sm'>No node selected</p>
			</Section>
		)
	}
...
function Palette() {
	...
	return (
		<Section title='Toolbar'>
			<Accordion multiple defaultValue={sections.map((s) => s.kind)}>
				{sections.map((section) => (
					...
					>
						<AccordionTrigger className='py-2 font-medium text-muted-foreground text-xs hover:no-underline'>
							{section.label}
						</AccordionTrigger>
